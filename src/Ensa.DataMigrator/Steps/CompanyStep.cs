        ...
        await using (var reader = await command.ExecuteReaderAsync(cancellationToken))
        {
            while (await reader.ReadAsync(cancellationToken))
            {
                ...
                batch.Add((legacyId, new Company
                {
                    ...
                    TaxOffice = Fit(context, "Company", "TaxOffice", Text(reader, "VergiDairesi")),
        ...
        await using (var reader = await command.ExecuteReaderAsync(cancellationToken))
        {
            while (await reader.ReadAsync(cancellationToken))
            {
                ...
                batch.Add((legacyId, new Company
                {
                    ...
                    SpecialistVisitMinutes = Int(reader, "ZiyaretUzman"),
                    PhysicianVisitMinutes = Int(reader, "ZiyaretDoktor"),
                    FirstMonthProgramIncluded = Bit(reader, "IlkAyPrograminaDahil"),
                    AreEmployeePasswordsSent = Int(reader, "SifreGonderildi") is > 0,
                    ...
                    UnofficialInvoiceAmount = Money(reader, "FaturaTutariKh"),
                    GroupContractAmount = Money(reader, "GRSozlesmeTutari"),
                    ExpectedPaymentAmount = Money(reader, "OdenecekRakam"),
                    PaymentDate = Date(reader, "OdemeTarihi"),
                    IsQuoteVatIncluded = string.Equals(Text(reader, "TeklifKdvDahil"), "1", StringComparison.Ordinal),
